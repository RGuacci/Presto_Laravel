...
class AnalyzeArticleImage implements ShouldBeUnique, ShouldQueue
{
    use Queueable;
    ...
    public function handle(ImageLabelAnalyzer $imageLabelAnalyzer): void
    {
        $articleImage = ArticleImage::query()->find($this->articleImageId);

        if ($articleImage === null || $articleImage->vision_status === ArticleImage::VISION_READY) {
            return;
        }

        $articleImage->update([
            'vision_status' => ArticleImage::VISION_PROCESSING,
        ]);
        ...
        $articleImage->update([
            'vision_labels' => $imageLabelAnalyzer->analyze($disk->get($articleImage->path)),
    ...
    public function failed(?Throwable $exception): void
    {
        ArticleImage::query()
            ...
            ->update(['vision_status' => ArticleImage::VISION_FAILED]);

        Log::error('Google Vision article image analysis failed.', [
